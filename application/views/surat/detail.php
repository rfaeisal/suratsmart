...
<div class="d-flex align-items-center gap-2 mb-3">
  ...
  <h5 class="mb-0 fw-semibold">Detail Surat</h5>
  ...
  <div class="col-lg-8">
    <div class="card border-0 shadow-sm mb-3">
      <div class="card-header bg-white d-flex justify-content-between align-items-center">
        <span class="fw-semibold">Informasi Surat</span>
        <span class="text-muted small font-monospace"><?= htmlspecialchars($item->nomor_surat) ?></span>
      </div>
      <div class="card-body">
        <dl class="row mb-0">
          <dt class="col-4 text-muted">Jenis Surat</dt>
          <dd class="col-8"><?= htmlspecialchars($item->jenis_surat) ?></dd>
          ...
          <dt class="col-4 text-muted">Dibuat</dt>
          <dd class="col-8"><?= tgl_indonesia($item->created_at) . date(' H:i', strtotime($item->created_at)) ?></dd>
        </dl>
      </div>
    </div>

    <div class="card border-0 shadow-sm">
      <div class="card-header bg-white fw-semibold">Isi Surat</div>
      <div class="card-body">
        <div style="white-space: pre-wrap; font-family: inherit; line-height: 1.8"><?= htmlspecialchars($item->isi) ?></div>
      </div>
    </div>
  </div>
  ...
  <div class="col-lg-4">
    <div class="card border-0 shadow-sm mb-3">
      ...
      <div class="card-body d-grid gap-2">
        ...
        <a href="<?= site_url('surat/edit/' . $item->id) ?>" class="btn btn-outline-primary btn-sm">
          <i class="bi bi-pencil me-1"></i>Edit Surat
        </a>
        <?php endif; ?>

        <?= form_open("surat/generate_pdf/{$item->id}") ?>
        ...
        <a href="<?= site_url('surat/download_pdf/' . $item->id) ?>" class="btn btn-success btn-sm">
          <i class="bi bi-download me-1"></i>Download PDF
        </a>
        <?php endif; ?>

        <?php if ($item->status === 'draft'): ?>
        <?= form_open("surat/delete/{$item->id}", ['onsubmit' => 'return confirm("Hapus surat ini?")']) ?>
    ...
    <div class="card border-0 shadow-sm mb-3 border-warning border-opacity-50">
      ...
      <div class="card-body">
        ...
        <?= form_open("surat/kirim/{$item->id}") ?>
          ...
          <button type="submit" class="btn btn-warning btn-sm w-100 fw-semibold" onclick="return confirm('Kirim surat ini ke ApprovalSmart?')">
    ...
    <div class="alert alert-danger py-2 small">
      ...
      <a href="<?= site_url('approval_log?module_type=surat&direction=outbound&status=gagal') ?>" class="alert-link">Lihat Log</a>
    </div>
    <?php endif; ?>
